...
@csrf_exempt
def actualizar_cantidad_picking(request):
    # try:
    ...
    if request.method == 'PUT':
        ...
        iddetpripack = request_data['iddetpripack'] if 'iddetpripack' in request_data else None
        cantidad = request_data['cantidad'] if 'cantidad' in request_data else None
            
        sp = '''

                SET NOCOUNT ON
                EXEC [web].[spi_prcActualizarCantidadesLineaPicking]  %s, %s
                '''
        try:

            response = exec_query(
                sp, (iddetpripack,cantidad,), database=database)
