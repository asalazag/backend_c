...
@csrf_exempt
def getubicacionesxCaja(request):
    # try:
    ...
    if request.method == 'GET':
        ...
        caja = request.GET['caja']  if 'caja' in request.GET else ''
        if caja == '':
            return JsonResponse({"message" : "caja is required"}, safe=False, status=405)
        
        sp = '''

                SET NOCOUNT ON
                EXEC [web].[usp_ObtenerUbicacionesDeCaja]  %s
                '''
        try:
            response = exec_query(
                sp, (caja,), database=database)
