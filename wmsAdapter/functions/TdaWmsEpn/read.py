...
def read_epn(request, db_name):
        ...
        if 'fecha_inicial' in params:
            current_date = datetime.strptime(params['fecha_inicial'][0] , '%Y-%m-%d')
            current_date = current_date + timedelta(days=1)
            fecha_inicial = current_date.strftime('%Y-%m-%d')
        ...
        try:    
            ...
            response = TdaWmsEpn.objects.using(db_name).filter(query, fechatransferencia__range=[fecha_inicial, fecha_final]).order_by('-fechatransferencia')[:1000]
            return response
