...
def readFilterPrv(request, db_name):
        ...
        try:    
            ...
            response = TdaWmsPrv.objects.using(db_name).filter(query, fecharegistro__range=[fecha_inicial, fecha_final]).order_by('-fecharegistro')[:10]
            return response
